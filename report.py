...
def extract_video_metadata(video_path):
    ...
    try:
        ...
        try:
            ...
            structure_analysis = {"signature_id": "Unknown", "signature_hash": "Unknown"}
            signature_structure = {"ftyp": "100.00 %", "moov": "42.48 %", "mvhd": "39.23 %"}
            generation_history = {"last_generation": "Unknown"}
            if "youtube" in writing_app.lower() or "dl" in writing_app.lower():
                generation_history["last_generation"] = "youtube-dl (\"Best\" Format)"
...
def is_not_black_image(image_path, threshold=0.01):
    """Check if image is not completely black (or nearly so)"""
    try:
...
def find_mri_images(data, base_dir):
    """Find MRI GAN images in the output directory and filter out black ones"""
    mri_images = []
...
def generate_html_report(data, video_metadata, args):
    ...
    <head>
        ...
        <style>
            /* CSS styles omitted for brevity; they remain unchanged */
            body {{
            ...
            .card h2 {{ border-bottom: 1px solid 
            ...
            .dark-theme .model-card {{ background-color: 
            .light-theme .model-card {{ background-color: 
            .confidence-meter {{ height: 20px; border-radius: 10px; margin: 10px 0; background-color: 
            .confidence-fill {{ height: 100%; transition: width 1s ease-in-out; }}
            .fake-confidence {{ background-color: 
            .real-confidence {{ background-color: 
            ...
            .tab-buttons {{ display: flex; overflow-x: auto; border-bottom: 1px solid 
            ...
            .light-theme .tab-button {{ color: 
            .tab-button.active {{ opacity: 1; border-bottom: 3px solid 
            .tab-button:hover {{ opacity: 0.8; }}
            .tab-content {{ display: none; }}
            .tab-content.active {{ display: block; animation: fadeIn 0.5s ease-out; }}
            .heatmap-gallery, .mri-gallery {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; margin-top: 20px; }}
            .image-card {{ border-radius: 8px; overflow: hidden; transition: transform 0.3s ease; }}
            .image-card:hover {{ transform: scale(1.05); }}
            .dark-theme .image-card {{ background-color: 
            .light-theme .image-card {{ background-color: 
            ...
            .lightbox-close:hover, .lightbox-close:focus {{ color: 
            .footer {{ text-align: center; margin-top: 40px; padding: 20px; border-top: 1px solid 
            table {{ width: 100%; border-collapse: collapse; margin: 20px 0; }}
            th, td {{ padding: 10px; text-align: left; border: 1px solid 
            .dark-theme th {{ background-color: 
            .light-theme th {{ background-color: 
            .metadata-section {{ background-color: 
            .metadata-row:nth-child(even) {{ background-color: rgba(255, 255, 255, 0.05); }}
            .warning-text {{ background-color: rgba(255, 193, 7, 0.2); color: 
