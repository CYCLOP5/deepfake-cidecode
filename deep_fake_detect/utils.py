...
def norm_probability(pred, prob):
    if pred == 0:
        ...
        return prob
def pred_strategy(num_fake_frames, num_real_frames, total_number_frames, fake_fraction=0.30, fake_prob=0, real_prob=0):
    if num_fake_frames >= (fake_fraction * total_number_frames) and fake_prob > real_prob:
        return 1  
    return 0  
...
def gen_roc(ground_truth, probability, model_roc_png):
    ...
    plt.close(fig)
def gen_report_for_per_frame_model(per_frame_csv=None, log_dir=None, report_type=None, prob_threshold_fake=0.55,
                                   prob_threshold_real=0.55, fake_fraction=0.30, log_kind=None, model_params=None):
